...
class App extends Component {
  gamestate (scene) {
    ...
    scene.addEventListener('componentinitialized', function initState (evt) {
      var store;
      if (evt.detail.name !== 'gamestate') { return; }
      store = scene.components.gamestate.store;
      store.subscribe(function subscribe () {
        self.setState(store.getState());
      });
      scene.removeEventListener('componentinitialized', initState);
    });
  }
  ...
  render () {
    ...
    return (
      <a-scene avatar-replayer="src: recordings/scaleClone.json" html-exporter gamestate
               primitive-scaler="entity: {state.app.activePrimitiveId}"
               ref={this.gamestate.bind(this)}>
